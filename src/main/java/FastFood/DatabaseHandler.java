...
public class DatabaseHandler {
    ...
    public static void updateProduit(int id, String newNom, double newPrix, String newImageOrNull) {
        if (newNom == null || newNom.trim().isEmpty()) return;
    ...
    public static void enregistrerCommande(String table, double total, String details) {
        ...
        try (Connection conn = connect(); PreparedStatement p = conn.prepareStatement(sql)) {
            ...
            logAction("system", "add_commande",
                    "Nouvelle commande table=" + table + ", total=" + total);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static double getStatsPeriode(String periode) {
        String query = switch (periode) {
            case "Jour" ->
                    "SELECT COALESCE(SUM(total),0) FROM commandes WHERE DATE(date) = CURDATE()";

            case "Semaine" ->
                    "SELECT COALESCE(SUM(total),0) FROM commandes WHERE date >= NOW() - INTERVAL 7 DAY";

            case "SemainePrecedente" ->
                    "SELECT COALESCE(SUM(total),0) FROM commandes " +
                    "WHERE date >= NOW() - INTERVAL 14 DAY AND date < NOW() - INTERVAL 7 DAY";

            case "Mois" ->
                    "SELECT COALESCE(SUM(total),0) FROM commandes WHERE date >= NOW() - INTERVAL 1 MONTH";
    ...
    public static int getNombreCommandesPeriode(String periode) {
        String query = switch (periode) {
            case "Jour" ->
                    "SELECT COUNT(*) FROM commandes WHERE DATE(date) = CURDATE()";

            case "Semaine" ->
                    "SELECT COUNT(*) FROM commandes WHERE date >= NOW() - INTERVAL 7 DAY";

            case "SemainePrecedente" ->
                    "SELECT COUNT(*) FROM commandes WHERE date >= NOW() - INTERVAL 14 DAY AND date < NOW() - INTERVAL 7 DAY";

            case "Mois" ->
                    "SELECT COUNT(*) FROM commandes WHERE date >= NOW() - INTERVAL 1 MONTH";
 ...
 // ================= NOUVELLES MÉTHODES POUR L'API FLUTTER =================

    public static boolean addProduit1(String nom, double prix, String categorieNom) {
        // 1. On cherche d'abord l'ID de la catégorie à partir de son nom
        ...
        try (Connection conn = connect()) {
            ...
            // Si la catégorie n'existe pas, on peut soit l'erreur, soit la créer. 
            // Ici on suppose qu'elle existe (Burger, Boisson, etc.)
            if (catId == -1) return false;

            // 2. On insère le produit
            try (PreparedStatement p = conn.prepareStatement(sqlInsert)) {
                p.setString(1, nom);
                p.setDouble(2, prix);
                p.setInt(3, catId);
                p.setInt(4, 0); // Ordre par défaut
                p.executeUpdate();
                
                logAction("API_Flutter", "add_produit", "Produit ajouté via l'app : " + nom);
                return true;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static boolean deleteProduit1(int id) {
        String sql = "DELETE FROM produits WHERE id = ?";
        try (Connection conn = connect(); PreparedStatement p = conn.prepareStatement(sql)) {
            p.setInt(1, id);
            ...
            if (rows > 0) {
                logAction("API_Flutter", "delete_produit", "Produit supprimé ID : " + id);
                return true;
            }
            return false;
        } catch (SQLException e) {
	...
	public static boolean addProduit(String nom, double prix, String categorie) {
		// TODO Auto-generated method stub
		return false;
	}
}